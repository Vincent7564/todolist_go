...
export default function Home() {
  ...
  const addData = async() =>{
    try{
      const res = await axios.post("http://127.0.0.1:3000/insert",{
        title : title,
        ...
        <div className="rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
          <div className="mb-4">
            ...
            <div className="flex mt-4">
              <input onChange={handleTitleChange} className="text-black shadow appearance-none border rounded w-full py-2 px-3 mr-4 text-grey-darker" placeholder="Add Todo" />
              <input onChange={handleDescriptionChange} className="text-black shadow appearance-none border rounded w-full py-2 px-3 mr-4 text-grey-darker" placeholder="Todo Description" />
              <button onClick={addData} className="flex-no-shrink p-2 border-2 rounded text-teal border-teal hover:text-white hover:bg-teal">Add</button>
            </div>
          </div>
          <div>
            {taskList ? (
